...
window.onload = function() {
	...
	canvas.width = ((newmap.width* tilesize) * Math.sin(45 * Math.PI / 180)) + ((newmap.height* tilesize) * Math.cos(45 * Math.PI / 180));
	canvas.height = ((newmap.width* tilesize) * Math.cos(45 * Math.PI / 180)) + ((newmap.height* tilesize) * Math.sin(45 * Math.PI / 180));
	var context = canvas.getContext('2d');
	//context.translate((canvas.width / 2) * Math.cos(45 * Math.PI / 180), (canvas.height / 2) * Math.sin(45 * Math.PI / 180));
	var trans_x = canvas.width / 2;
	var trans_y = canvas.height / 4;
	console.log(trans_x);
	console.log(trans_y);
	//context.translate(trans_x, trans_y);
	context.translate(tilesize + tilesize, tilesize);
	//context.scale(1,0.5);
	context.rotate(36.2 * Math.PI / 180);
	newmap.generate();
	newmap.draw();
	vw.drawView();
};
